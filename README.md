...
1. **Автоматическое получение заголовков**:
   - При первом запуске библиотека использует Playwright для запуска Chromium в виртуальном дисплее
   - Происходит имитация реального пользователя на сайте duck.ai
   - Система автоматически собирает необходимые заголовки для API-запросов

2. **Взаимодействие с API**:
   - Библиотека формирует правильно структурированные запросы к DuckDuckGo API
   - Поддерживает потоковую передачу ответов (streaming)
   - Автоматически обрабатывает ошибки и обновляет заголовки при необходимости

3. **API-сервис**:
   - Предоставляет RESTful интерфейс для интеграции с любыми приложениями
   - Поддерживает загрузку изображений для мультимодальных запросов
   - Автоматически обновляет заголовки при возникновении ошибок аутентификации
...
- **Только Linux (если получение заголовков требуется в headless режиме)** (из-за зависимостей `xvfbwrapper`)
